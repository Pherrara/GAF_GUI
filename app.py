...
mode = st.sidebar.radio("Mode", ["Calibration", "Measurement"], index=0)

# --- Shared: channel, film prefixes, extension, segmentation thresholds ---
st.sidebar.divider()
...
# --- Calibration-only sidebar options ---
if mode == "Calibration":
    st.sidebar.divider()
    ...
    if os.path.isfile(x_values_file):
        try:
            with open(x_values_file, "r") as f:
                lines_mu = [line for line in f if line.strip()]
            st.sidebar.markdown(f"✅ `{x_values_file}` found with **{len(lines_mu)} values**.")
...
# --- Measurement-only sidebar options ---
if mode == "Measurement":
    st.sidebar.divider()
...
with st.expander("ℹ️ Show Info About This App"):
    ...
    **Calibration Requirements**:
    - Place the numbered images (e.g. `.tif`) in the selected folder and select the accurate prefixes (e.g. `irr`, `velo`).
    - `MU_values.dat` must be available in the app folder (or uploaded via the sidebar). Each line should contain the MU delivered for each film, in order.
    - The Dose Conversion Coefficient can be provided via `dcc.dat` — this is how much cGy are received at the film geometry per 100 MU. Defaults to 100 cGy if not found.
    - Adjust polynomial fit order and number of fit points as needed.
    - After running, save the run for direct comparison across sessions, or download the calibration `.dat` file for use in future measurements.

    **Measurement Requirements**:
    - Place the numbered images (e.g. `.tif`) in the selected folder and select the accurate prefixes.
    - Upload a calibration `.dat` file produced by a previous calibration run.

    **Workflow Summary**:
    - Automatically segment irradiated and unirradiated film scans
    - Compute mean pixel value and standard deviation within the segmented region
    - Compute Optical Density and fit a calibration curve to dose using a weighted polynomial
    - Compare multiple calibration runs visually
    - Estimate dose from new films using a loaded calibration

    **Use the Mode selector at the top of the sidebar to switch between Calibration and Measurement.**
    """)
    ...
    if st.session_state.get("run_calibration", False):
        ...
        if len(irradiated_list) != len(velo_list):
            ...
            for name_irr, name_vel in zip(irradiated_list, velo_list):
                ...
                seg_figs_calib.append(display_overlay(img_irr, final_mask_irr, f"Final segmentation: {name_irr}"))
                ...
                seg_figs_calib.append(display_overlay(img_vel, final_mask_vel, f"Final segmentation: {name_vel}"))
            ...
            if st.button("Save for Direct Comparison"):
            ...
            st.download_button("Download Calibration Run Coefficients (.dat)",
            ...
            st.subheader("Film Segmentation (Calibration)")
            for fig in seg_figs_calib:
                st.pyplot(fig)
...
if mode == "Measurement":
    ...
    uploaded_calib_file = st.file_uploader("Upload calibration .dat file", type=["dat", "txt"])
    if uploaded_calib_file is not None:
        try:
            coeffs = np.loadtxt(uploaded_calib_file)
            poly_fit = np.poly1d(coeffs[::-1])
            st.success("Calibration loaded successfully.")
        except Exception as e:
            st.error(f"Failed to parse calibration file: {e}")
    ...
    if st.session_state.get("make_measurement", False):
        ...
        for f_irr, f_vel in zip(irradiated_list, velo_list):
            ...
            img_irr, mask_irr = load_image_and_segment_white(path_irr, white_threshold_percent)
            _, mean_i, std_i, final_mask_i = final_segmentation(img_irr, tolerance_percent, mask_irr, channel_code)
            mean_irr.append(mean_i)
            std_irr.append(std_i)
            seg_figs_meas.append(display_overlay(img_irr, final_mask_i, f"Final segmentation: {f_irr}"))

            img_vel, mask_vel = load_image_and_segment_white(path_vel, white_threshold_percent)
            _, mean_v, std_v, final_mask_v = final_segmentation(img_vel, tolerance_percent, mask_vel, channel_code)
            mean_velo.append(mean_v)
            std_velo.append(std_v)
            seg_figs_meas.append(display_overlay(img_vel, final_mask_v, f"Final segmentation: {f_vel}"))
        ...
        st.subheader("Film Segmentation (Measurement)")
        for fig in seg_figs_meas:
            st.pyplot(fig)
